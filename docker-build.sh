...
IMAGE_NAME="${IMAGE_NAME:-email-mcp-server}"
...
if [ "$TEST_ONLY" = true ]; then
    ...
    echo -e "${BLUE}📦 Building Docker image...${NC}"
        # Determine base image for build (allow override via env)
        BASE_IMAGE_ENV="${BASE_IMAGE:-node:22-slim}"
        echo -e "${YELLOW}Using base image: ${BASE_IMAGE_ENV}${NC}"

        # Try to pull the base image to detect connectivity / availability
        if docker pull "${BASE_IMAGE_ENV}" > /dev/null 2>&1; then
            BUILD_ARGS+=("--build-arg" "BASE_IMAGE=${BASE_IMAGE_ENV}")
        else
            echo -e "${YELLOW}⚠️  Could not pull ${BASE_IMAGE_ENV}. Falling back to node:20-slim for build.${NC}"
            BUILD_ARGS+=("--build-arg" "BASE_IMAGE=node:20-slim")
        fi
